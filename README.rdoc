...
Ruby 1.8 is deprecated, and no longer supported.
...
The key file and a self-signed certificate can be genterated automatically during prax installation. THey can be re-generated if needed by deleting the <tt>server.*</tt> files in <tt>/opt/prax/ssl/</tt> then running 
the <tt>generate.rb</tt> script. 

  $ cd /opt/prax/ssl; rm server.*; bash ./generate.sh

Since this certificate is only being used for local 
development, we just skip most of the prompts.

Optionally you can use your own certificate by copying the files to:

  $ cp <path_to_crt_file> /opt/prax/ssl/server.crt
  $ cp <path_to_key_file> /opt/prax/ssl/server.key
...
**This will result in a security warning because the browser cannot validate it 
against a local, trusted issuer certificate.** For development, this should not be of much 
concert and you can add an exception for this in your browser.

Now you can point your browser to <tt>https://myapp.dev/</tt> and test the app via SSL.

=== Custom Configuration

You can modify the default behavior by creating a <tt>~/.praxconfig</tt> file. 

These are the defaults:

  PRAX_ROOT=/opt/prax/libexec
  PRAX_HOSTS=~/.prax
  PRAX_DEBUG=false
...
<tt>~/.bashrc</tt> for example, but we aware that it will restart Prax (and all
