...
NOTE: a full rewrite of Prax is available in the unstable branch and comes with
a few more features than available in the current master branch. You are
encouraged to try it out —it may be more compatible with the different
Ruby version managers, and has an autokill monitor to kill the apps after a TLL,
among other things.
