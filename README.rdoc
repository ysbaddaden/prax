...
Unlike pow, which uses the Node.js single-thread evented model to run its apps,
prax uses multiple Ruby threads per app to serve requests. Because of this, problems
will arise if your app is not threadsafe. If this affects you, the current
workaround is to make prax only use a single thread to service each app by adding
the following in your `~/.praxconfig`:

  export PRAX_APP_THREADS=1
